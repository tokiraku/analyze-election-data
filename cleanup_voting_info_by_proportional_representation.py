...
def load_excel_sheets():
    """
    埼玉投票情報_比例代表.xlsファイルの全シートを確認し、指定されたシートを読み込む関数
    """
    # ファイルパスを取得
...
def clean_and_save_sheet_data(sheet_data, sheet_name):
    ...
    output_dir = "output_info_proportional"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    
    csv_filename = os.path.join(output_dir, f"比例代表_{sheet_name}_最終版.csv")
...
def create_summary_report(all_processed_data):
    ...
    """
    print(f"\n{'='*60}")
    print("📊 投票情報処理結果サマリーレポート（比例代表版）")
    print(f"{'='*60}")
    
    output_dir = "output_info_proportional"
    summary_file = os.path.join(output_dir, "比例代表_投票情報処理サマリーレポート.txt")
    
    with open(summary_file, 'w', encoding='utf-8') as f:
        f.write("埼玉投票情報分析（比例代表版） - 処理サマリーレポート\n")
        ...
        for sheet_name, df in all_processed_data.items():
            if df is not None:
                ...
                f.write(f"  - 出力ファイル: 比例代表_{sheet_name}_最終版.csv\n")
                f.write("\n")
...
def main():
    ...
    メイン関数
    """
    print("=== 埼玉投票情報分析プログラム（比例代表版） ===")
    print("📂 出力ディレクトリ:")
    print("   - tmp_info_proportional/: クリーンアップ前のデータ")
    print("   - output_info_proportional/: 最終的な処理済みデータ")
    print("")
    ...
    print(f"\n{'='*60}")
    print("🎉 比例代表投票情報データの処理が完了しました！")
    print("📁 出力ファイルの確認:")
    print("   - tmp_info_proportional/: 元データのバックアップ")
    print("   - output_info_proportional/: 重複削除済みの最終データ")
    print("   - output_info_proportional/比例代表_投票情報処理サマリーレポート.txt: 処理結果の概要")
    print(f"{'='*60}")
